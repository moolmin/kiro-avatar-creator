...
export default function SavePage() {
  ...
  const [avatarData, setAvatarData] = useState<{ image: string; config: any; timestamp: number } | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    // Retrieve saved avatar from global store
    const data = savedAvatarStore.getSavedAvatar();
    if (data) {
      setAvatarData(data);
    }
    setIsLoading(false);
  ...
  const handleDownload = () => {
    ...
    link.href = avatarData.image;
    link.download = `kiroween-avatar-${Date.now()}.png`;
      ...
      <main className="min-h-screen flex items-center justify-center bg-halloween-gradient">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            <p className="text-xl font-medium text-gray-800 animate-pulse">Loading...</p>
          </div>
        </div>
      </main>
    );
  ...
  if (!avatarData) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-halloween-gradient">
        <div className="text-center">
          <p className="text-xl font-medium text-gray-800 mb-4">No avatar found</p>
          <button
            onClick={handleRetry}
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-primary-purple text-white text-lg font-semibold rounded-xl hover:bg-purple-700 transition-all duration-300 hover:scale-105 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-purple-300"
              >
                <svg
                  className="h-5 w-5"
                  viewBox="0 0 20 20"
              ...
              <button
                onClick={handleRetry}
                className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-white text-gray-900 text-lg font-semibold rounded-xl border-2 border-gray-300 hover:bg-gray-50 transition-all duration-300 hover:scale-105 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-gray-300"
              >
                <svg
                  className="h-5 w-5"
                  viewBox="0 0 20 20"
